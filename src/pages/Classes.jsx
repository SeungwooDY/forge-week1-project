...
    function buildTimeTimestamp(timeValue) {
        if (!timeValue) {
            return null
        }

        let cleanedTime = timeValue.trim().toUpperCase()
        let hours
        let minutes
...
            cleanedTime = cleanedTime.replace('AM', '').replace('PM', '').trim()
...
    function timestampToDisplayTime(ts) {
        if (!ts) {
            return ''
        }
...
    function timestampToComparableDate(ts) {
        if (!ts) {
            return null
        }
...
        const startTimestamp = buildTimeTimestamp(form.start_time)
        const endTimestamp = buildTimeTimestamp(form.end_time)

        console.log('start time value:', form.start_time)
        console.log('end time value:', form.end_time)
...
        console.log('newClass being saved:', newClass)
...
        if (form.cgrade && (form.cgrade < 1 || form.cgrade > 5)) {
            next.cgrade = 'Grade must be between 1 and 5'
        }
...
        const startDate = timestampToComparableDate(startTimestamp)
        const endDate = timestampToComparableDate(endTimestamp)

        if (form.start_time && !startTimestamp) {
            next.start_time = 'Invalid start time'
        }

        if (form.end_time && !endTimestamp) {
            next.end_time = 'Invalid end time'
        }
...
            start_time: c.start_time ? timestampToDisplayTime(c.start_time) : '',
            end_time: c.end_time ? timestampToDisplayTime(c.end_time) : '',
...
                <button onClick={handleAddClass} className="px-4 py-2 border-2 rounded-2xl">
                    Add Class
                </button>
...
                                        ? `${timestampToDisplayTime(c.start_time)} – ${timestampToDisplayTime(c.end_time)}`
                                        : '–'}
                                </td>
...
                    >
                        <div
                            className="bg-white rounded-2xl shadow-xl p-6 max-w-2xl w-full max-h-[90vh] overflow-auto"
                            onClick={(e) => e.stopPropagation()}
                        >
...
                                {!editTarget && (
                                    <div className="flex flex-col">
                                        <label className={label}>Initial Student</label>
                                        <select
                                            className={input}
                                            name="student_sid"
