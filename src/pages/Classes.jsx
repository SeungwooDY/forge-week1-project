...
export default function Classes() {
    ...
    function buildTimeDate(timeValue) {
        if (!timeValue) {
            return null
        }

        let cleanedTime = timeValue.trim()
        ...
        return new Date(`1970-01-01T${cleanedTime}:00`)
    }
    ...
    const handleSubmit = async () => {
        ...
        const newClass = {
            ...
            start_time: buildTimeDate(form.start_time),
            end_time: buildTimeDate(form.end_time),
    ...
    const isValid = () => {
        ...
        if (form.start_time && form.end_time && buildTimeDate(form.start_time) >= buildTimeDate(form.end_time)) {
                            ...
                            <tr key={c.id} className="border-b border-slate-100 hover:bg-slate-50">
                                ...
                                <td className="px-4 py-3 text-sm text-slate-600">
                                    {c.start_time && c.end_time
                                        ? `${c.start_time.toDate().toLocaleTimeString([], {
                                              hour: 'numeric',
                                              minute: '2-digit',
                                          })} – ${c.end_time.toDate().toLocaleTimeString([], {
                                              hour: 'numeric',
                                              minute: '2-digit',
                                          })}`
                                        : '–'}
